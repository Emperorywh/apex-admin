...
const SettingsDrawer: React.FC<SettingsDrawerProps> = ({ children, onSettingChange }) => {
	...
	const handleUpdateSettings = <K extends keyof SettingsState>(
		key: K,
				...
				<div className="flex flex-col gap-6">
					...
					<div>
						...
						<div className="flex gap-4">
							...
							>
								<SunOutlined
									className={`text-xl transition-transform duration-300 ${settings.themeMode === 'light' ? 'scale-110' : 'group-hover:scale-110'}`}
								/>
							</div>
							<div
								className={`group flex-1 flex items-center justify-center h-16 rounded-xl border-2 cursor-pointer transition-all duration-300 ${
							...
							>
								<MoonOutlined
									className={`text-xl transition-transform duration-300 ${settings.themeMode === 'dark' ? 'scale-110' : 'group-hover:scale-110'}`}
								/>
							</div>
						</div>
					</div>
							...
							<Row align="middle" gutter={16}>
								...
								<Col flex="auto">
									<Slider
										...
										onChange={(value) =>
											handleUpdateSettings('borderRadius', value)
										}
										tooltip={{ formatter: (value) => `${value}px` }}
									/>
								</Col>
								<Col flex="none">
									<InputNumber
										...
										value={settings.borderRadius}
										onChange={(value) =>
											handleUpdateSettings('borderRadius', value || 0)
										}
									/>
								</Col>
							</Row>
