		...
		},
		{
			...
			render: (text: string, record: PageData) => (
				<span>
					{text}{' '}
					<span
						className={`ml-2 text-xs ${record.viewsChange.includes('↑') ? 'text-green-500' : 'text-red-500'}`}
					>
						{record.viewsChange}
					</span>
				</span>
		...
		},
		{
			...
			render: (text: string, record: PageData) => (
				<span>
					{text}{' '}
					<span
						className={`ml-2 text-xs ${record.uniqueChange.includes('↑') ? 'text-green-500' : 'text-red-500'}`}
					>
						{record.uniqueChange}
					</span>
				</span>
