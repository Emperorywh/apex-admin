"""组织模块 Use Case — Application 层应用服务（SPEC 5.2 / 5.6 / 5.7 / 14.1 / 18.2）.
...
SPEC 5.7 审计:
  - 部门变更通过 AuditPort 写审计，并与业务事务共同提交
    （SPEC 5.7 / 18.2）。
...
from app.modules.org.models import Department, DepartmentStatus, DepartmentTreeNode
...
class OrgUseCase:
    ...
    async def delete_department(
        ...
        - 存在用户关联时拒绝删除（``ORG.DEPT_HAS_USERS``）。
          用户组织关系在 TASK-020 实现；当前无用户-部门关联数据。
        """
